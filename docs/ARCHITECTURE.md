...
2. **`ClientPreview.tsx`** (`getClientStyle`) — the read-only "Client Preview" modal. Aims to simulate how OTClient would actually lay the widget out (anchor resolution against a `widgetMap` of id/name → widget, default sizes per widget type, absolute positioning for anchored widgets). See [Client Preview & Runtime Parity](./CLIENT_PREVIEW.md) for known gaps vs. real OTClient behavior.
